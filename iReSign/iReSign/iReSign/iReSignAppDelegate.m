...
- (IBAction)resign:(id)sender {
    ...
    if ([certComboBox objectValue]) {
        ...
        else if ([[NSFileManager defaultManager] fileExistsAtPath:[sourcePath stringByAppendingPathComponent:kPayloadDirName]]) {
            [self proceedWithAppFolder];
        }
        else {
...
- (void)proceedWithAppFolder
{
    NSString * resigned = [sourcePath stringByAppendingString:@"_temp"];
    NSFileManager * fm = [NSFileManager defaultManager];
    if ([fm fileExistsAtPath:resigned]) {
        [fm removeItemAtPath:resigned error:NULL];
    }
    [fm copyItemAtPath:sourcePath toPath:resigned error:NULL];
    workingPath = resigned;
    
    NSLog(@"Unzipping done");
    [statusLabel setStringValue:@"Original app extracted"];
    
    [self doBundleIDChange:@"com.wutian.facebook"];
...
- (void)doCodeSigning {
    ...
    for (NSString *file in dirContents) {
        if ([[[file pathExtension] lowercaseString] isEqualToString:@"app"]) {
            ...
            NSLog(@"Found %@",appPath);
            appName = file;
            NSString * dylibsPath = [appPath stringByAppendingPathComponent:@"Dylibs"];
            ...
            if ([[NSFileManager defaultManager] fileExistsAtPath:dylibsPath]) {
                hasFrameworks = YES;
                NSArray * dylibsContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:dylibsPath error:nil];
                for (NSString * file in dylibsContents) {
                    [frameworks addObject:[dylibsPath stringByAppendingPathComponent:file]];
                }
            }
